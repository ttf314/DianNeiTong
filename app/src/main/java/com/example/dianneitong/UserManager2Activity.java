...
public class UserManager2Activity extends AppCompatActivity implements View.OnClickListener{
   ...
   /* private UserDao userDao;    // 用户数据库操作实例
    ...
    private LvUserinfoAdapter2 lvUserinfoAdapter;   // 用户信息数据适配器

    private ListView lv_user2;   // 用户列表组件

    private Handler mainHandler;   // 主线程
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_user_manager3);
    ...
    private void initView(){
        ...
        lv_user2 = findViewById(R.id.lv_user2);
    ...
    private void loadUserDb(){
        new Thread(new Runnable() {
            @Override
            public void run() {
                userInfos = userDao2.getAllUserList();   // 获取所有的用户数据
                Log.i("管理界面的数据", "用户数量："+userInfos.size());
                //回到主线程显示数据
    ...
    private void showLvData(){
        if(lvUserinfoAdapter==null){   // 首次加载时的操作
            lvUserinfoAdapter = new LvUserinfoAdapter2(this, userInfos);
            lv_user2.setAdapter(lvUserinfoAdapter);
        }else{   // 更新数据时的操作
            lvUserinfoAdapter.setUserinfoList(userInfos);
        ...
        lvUserinfoAdapter.setOnEditBtnClickListener(new OnEditBtnClickListener() {
            ...
            public void onEditBtnClick(View view, int position) {
                ...
                Intent intent = new Intent(UserManager2Activity.this, UseEditActivity.class);
        ...
        lvUserinfoAdapter.setOnDelBtnClickListener(new OnDelBtnClickListener() {
            ...
            public void onDelBtnClick(View view, int position) {
                //  删除方法
                final UserInfo item = userInfos.get(position);
                new AlertDialog.Builder(UserManager2Activity.this)
                        .setTitle("删除确定")
    ...
    private void doDelUser(final int id){
        new Thread(new Runnable() {
            @Override
            public void run() {
                final int iRow = userDao2.delUser(id);
                mainHandler.post(new Runnable() {
                    @Override
                    public void run() {
                        loadUserDb();  // 重新加载数据
