...
import com.example.dianneitong.info.CarsInfo;
...
public class CarDao extends DBHelper {
     ...
     * @return int 影响的行数
     */
    public int addCars(CarsInfo item){
        int iRow = 0;
        int rmb=100;
        try{
            getConnection();   // 取得连接信息
            String sql = "insert into carinfo(id,foodname, money,num) values(?,?, ?, ?)";
            pStmt = conn.prepareStatement(sql);
            pStmt.setInt(1, item.getId());
            pStmt.setString(2, item.getFoodname());
            pStmt.setDouble(3, item.getMoney());
